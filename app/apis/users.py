...
create_user_fields = api.model('CreateUserModel', {
    'username': fields.String(description="Username", required=True),
    'email': fields.String(description="User's Email", required=True)
})

update_user_fields = api.model('CreateUserModel', {
...
class UserMain(Resource):
    ...
    def post(self):
        ...
        user = User(username=payload["username"], email=payload["email"], api_key=(payload["api_key"] if "api_key" in payload else ""))
        db.session.add(user)
        db.session.commit()
        return user, 200
